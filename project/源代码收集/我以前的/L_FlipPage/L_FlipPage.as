package L_FlipPage
{
	...
	*/
	...
	{		
		...
		var _mask_shadow:L_FlipPage_mask;
		...
		function init(w:uint=700,h:uint=500):void
		{			
			...
			_mask_shadow = new L_FlipPage_mask();

			addChild(_mask_flipingPage);
			addChild(_mask_newPage);
			addChild(_mask_page);
			addChild(_mask_shadow);
			...
			_page_shower.shadow_mask = _mask_shadow;
		...
		function draw_self(sp:Sprite):void
		{
		...
		{			
			...
			start_listen_mouse_inside_press();
		}
		...
		function show_moving(evt:L_Event):void
		{
			...
			_page_shower.shadow_show(_point_manager.shadow_position);
		}
		function start_flip_page(evt:L_Event):void
		{			
			_point_manager.stop_roll_end();
			stop_listen_point_roll_end();			
		...
		function start_drag_page(evt:L_Event):void
		{
			...
			stop_listen_mouse_inside_press();
		...
		function change_tween_end(evt:MouseEvent):void
		{
			_point_manager.change_tween_end(mouseX, mouseY);
		}
		...
		function mouse_roll_over(evt:L_Event):void
		{
			...
			_point_manager.point_type = e_args.position;	
			
			start_listen_point_roll_end();
			
			_point_manager.start_roll_page(mouseX, mouseY);
		...
		{			
			...
			start_listen_point_roll_back();
			
			_point_manager.start_roll_back();
		}
		
		function point_roll_end(evt:L_Event):void
		{			
			stop_listen_point_roll_end();
		...
		function point_roll_back(evt:L_Event):void
		{
			stop_listen_point_roll_back();
		...
		function mask_draw():void
		{
			...
			_mask_shadow.mask_draw(_point_manager.mask_shadow_points);
		}		
		...
		function start_listen_drag_back():void
		{
			...
			{
				stage.addEventListener(MouseEvent.MOUSE_MOVE, change_tween_end)
			}
		}
		...
		function stop_listen_drag_back():void
		{
			...
			if (stage.hasEventListener(MouseEvent.MOUSE_MOVE))			
			{
				stage.removeEventListener(MouseEvent.MOUSE_MOVE, change_tween_end)
			}
		}		
		...
		function start_listen_point_roll_end():void
		{
			if (!_point_manager.hasEventListener(L_Event.POINT_ROLL_END))
			{
				_point_manager.addEventListener(L_Event.POINT_ROLL_END, point_roll_end);
			}
			if (!stage.hasEventListener(MouseEvent.MOUSE_MOVE))			
			{
				stage.addEventListener(MouseEvent.MOUSE_MOVE, change_tween_end)
		...
		function stop_listen_point_roll_end():void
		{
			if (_point_manager.hasEventListener(L_Event.POINT_ROLL_END))
			{
				_point_manager.removeEventListener(L_Event.POINT_ROLL_END, point_roll_end);				
			}
			if (stage.hasEventListener(MouseEvent.MOUSE_MOVE))			
			{
				stage.removeEventListener(MouseEvent.MOUSE_MOVE, change_tween_end)
		...
		function start_listen_point_roll_back():void
		{
			if (!_point_manager.hasEventListener(L_Event.POINT_ROLL_BACK))
			{
				_point_manager.addEventListener(L_Event.POINT_ROLL_BACK, point_roll_back);
			}
		}
		
		function stop_listen_point_roll_back():void
		{
			if (_point_manager.hasEventListener(L_Event.POINT_ROLL_BACK))
			{
				_point_manager.removeEventListener(L_Event.POINT_ROLL_BACK, point_roll_back);
			}
		}
		...
		function start_listen_mouse_inside_press():void
		{
		...
		function stop_listen_mouse_inside_press():void
		{
			if (_btn_manager.hasEventListener(L_Event.MOUSE_INSIDE_PRESS))
			{
		...
		{
			_point_manager.page_roll_over_seconds = value;				
		}
		
		public function set page_roll_back_seconds(value:Number):void
		{
			_point_manager.page_roll_back_seconds = value;					
		}
	}
}